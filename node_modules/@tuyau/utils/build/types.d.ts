...
type Serialize<T> = IsAny<T> extends true ? any : T extends JsonPrimitive | undefined ? T : T extends Map<any, any> | Set<any> ? Record<string, never> : T extends NonJsonPrimitive ? never : T extends {
    toJSON(): infer U;
} ? U : T extends [] ? [] : T extends [unknown, ...unknown[]] ? SerializeTuple<T> : T extends ReadonlyArray<infer U> ? (U extends NonJsonPrimitive ? null : Serialize<U>)[] : T extends object ? T extends {
    [key: string]: JsonPrimitive;
} ? T : SerializeObject<T> : never;
 ...
 */
type ConvertReturnTypeToRecordStatusResponse<T> = {
    [P in T as P extends {
        __status: infer S extends number;
    } ? S : 200]: P extends {
        __response: infer R;
    } ? R : P;
};
 ...
 */
type MakeTuyauResponse<T extends (...args: any) => any, HasSchema extends boolean = false> = Simplify<Serialize<ConvertReturnTypeToRecordStatusResponse<Awaited<ReturnType<T>>>>> & (HasSchema extends true ? {
    422: {
        errors: {
            message: string;
            rule: string;
 ...
 */
type MakeNonSerializedTuyauResponse<T extends (...args: any) => any, HasSchema extends boolean = false> = Simplify<ConvertReturnTypeToRecordStatusResponse<Awaited<ReturnType<T>>>> & (HasSchema extends true ? {
    422: {
        errors: {
            message: string;
            rule: string;
 ...
 */
type MakeTuyauRequest<T extends object> = MakeOptional<{
    [K in keyof T]: T[K] extends MultipartFile ? Blob | File | ReactNativeFile : T[K];
}>;

export type { ConvertReturnTypeToRecordStatusResponse, IsNever, MakeNonSerializedTuyauResponse, MakeOptional, MakeTuyauRequest, MakeTuyauResponse, Prettify, Serialize, Simplify };
